...
export async function findReturnByTracking(tracking) {
  ...
  if (!label && target.length >= MIN) {
    label = candidates.find((l) => {
      const t = normalize(l.tracking_number);
      return t.length >= MIN && (t.includes(target) || target.includes(t));
    });
  }

  // Dernier recours : les transporteurs impriment fréquemment le numéro
  // sans sa clé de contrôle finale, ou tronqué à ses premiers caractères.
  if (!label && target.length >= MIN) {
    label = candidates.find((l) => {
      const t = normalize(l.tracking_number);
      const short = Math.min(t.length, target.length) - 1;
      return short >= MIN && t.slice(0, short) === target.slice(0, short);
    });
  }

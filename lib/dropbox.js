...
//   return_type,exchange_product,exchange_size,return_reason,return_comment
//
...
export async function assignLabel(orderNumber, details = {}) {
  ...
  const apply = (row) => {
    ...
    row.return_reason = details.reason || '';
    // Le commentaire est libre : on neutralise virgules et sauts de ligne,
    // qui casseraient la structure du CSV.
    row.return_comment = String(details.comment || '').replace(/[,\r\n]+/g, ' ').trim();
  };
  ...
  if (existing) {
    const changed =
      ...
      existing.return_reason !== (details.reason || '');

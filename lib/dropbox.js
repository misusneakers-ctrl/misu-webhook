...
// Retrouve un retour à partir du numéro de suivi de son étiquette.
//
// Le code-barres scanné ne contient pas les espaces du CSV, et une étiquette
// transporteur porte souvent le numéro entouré de préfixes de routage. On
// compare donc des versions normalisées, puis on accepte l'inclusion dans
// un sens comme dans l'autre — avec une longueur minimale, sinon deux
// numéros proches se confondraient.
export async function findReturnByTracking(tracking) {
  const normalize = (v) => String(v || '').replace(/[^a-z0-9]/gi, '').toUpperCase();
  const target = normalize(tracking);
  if (!target) return null;

  const { rows: labels } = await readInventory();
  const candidates = labels.filter((l) => l.return_id && l.tracking_number);

  const MIN = 8;

  // Le code-barres d'une étiquette Chronopost n'est pas le numéro de suivi :
  // c'est un code de routage qui le CONTIENT, entouré d'un préfixe de tri et
  // d'une clé de contrôle. Exemple : le suivi XT268884225TS apparaît dans
  // 0075003XT268884225613400901C. On compare donc plusieurs formes du
  // numéro, dont sa version privée des lettres finales.
  const variants = (t) => {
    const n = normalize(t);
    const set = new Set([n]);
    const sansSuffixe = n.replace(/[A-Z]+$/, '');
    if (sansSuffixe.length >= MIN) set.add(sansSuffixe);
    const sansPrefixe = n.replace(/^[A-Z]+/, '');
    if (sansPrefixe.length >= MIN) set.add(sansPrefixe);
    return [...set];
  };

  let label = candidates.find((l) => normalize(l.tracking_number) === target);

  if (!label) {
    label = candidates.find((l) =>
      variants(l.tracking_number).some(
        (v) => v.length >= MIN && (target.includes(v) || v.includes(target))
      )
    );
  }
  ...
  const { rows: items } = await readReturns();
...
// Lien de téléchargement temporaire vers le PDF d'une étiquette.
//
// Le fichier se déplace au fil du cycle de vie (AVAILABLE, CONFIRMED,
// RECEIVED) et son nom change à l'assignation. Plutôt que de suivre sa
// position dans le CSV — qui se désynchroniserait au moindre déplacement
// manuel — on essaie les emplacements possibles jusqu'à en trouver un.
//
// Le lien renvoyé expire après quelques heures : il est fabriqué à chaque
// demande, jamais stocké ni envoyé par email.
export async function getLabelLink(returnId) {
  const { rows: labels } = await readInventory();
  const label = labels.find((l) => l.return_id === returnId);
  if (!label) return null;

  const cleanOrder = String(label.order_name || '').replace(/^#/, '');
  const n = label.label_number;
  const original = label.pdf_file || `${LABEL_PREFIX}${n}.pdf`;

  const candidates = [
    `${FOLDER_CONFIRMED}/${cleanOrder}_${returnId}_${original}`,
    `${FOLDER_RECEIVED}/${cleanOrder}_${returnId}_${original}`,
    `${FOLDER_AVAILABLE}/${original}`,
    `${FOLDER_CONFIRMED}/${original}`,
    `${FOLDER_RECEIVED}/${original}`
  ];

  for (const path of candidates) {
    const link = await temporaryLink(path);
    if (link) return { link, labelNumber: n, trackingNumber: label.tracking_number };
  }

  console.error(`Aucun PDF trouvé pour ${returnId} (étiquette ${n}).`);
  return null;
}
...
// Liste les PDF présents dans le dossier des étiquettes disponibles.
export async function listAvailableFiles() {
  const res = await fetch('https://api.dropboxapi.com/2/files/list_folder', {
    method: 'POST',
    headers: await dropboxHeaders({ 'Content-Type': 'application/json' }),
    body: apiArg({ path: FOLDER_AVAILABLE, limit: 500 })
  });

  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    throw new Error(`Lecture du dossier AVAILABLE refusée (HTTP ${res.status}) : ${detail}`);
  }

  const data = await res.json();
  const noms = (data.entries || [])
    .filter((e) => e['.tag'] === 'file' && /\.pdf$/i.test(e.name))
    .map((e) => e.name);

  // Le dossier peut contenir plus d'entrées que la première page.
  let cursor = data.has_more ? data.cursor : null;
  while (cursor) {
    const suite = await fetch('https://api.dropboxapi.com/2/files/list_folder/continue', {
      method: 'POST',
      headers: await dropboxHeaders({ 'Content-Type': 'application/json' }),
      body: apiArg({ cursor })
    });
    if (!suite.ok) break;
    const bloc = await suite.json();
    (bloc.entries || [])
      .filter((e) => e['.tag'] === 'file' && /\.pdf$/i.test(e.name))
      .forEach((e) => noms.push(e.name));
    cursor = bloc.has_more ? bloc.cursor : null;
  }

  return noms.sort();
}

// Télécharge un PDF du dossier des étiquettes disponibles.
export async function downloadFile(fileName) {
  const res = await fetch('https://content.dropboxapi.com/2/files/download', {
    method: 'POST',
    headers: await dropboxHeaders({
      'Dropbox-API-Arg': apiArg({ path: `${FOLDER_AVAILABLE}/${fileName}` })
    })
  });

  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    throw new Error(`Téléchargement de ${fileName} refusé (HTTP ${res.status}) : ${detail}`);
  }

  return Buffer.from(await res.arrayBuffer());
}

// Noms de fichiers déjà présents dans l'inventaire : évite les doublons
// quand l'import est relancé.
export async function getKnownFiles() {
  const { rows } = await readInventory();
  return new Set(rows.map((r) => r.pdf_file).filter(Boolean));
}

// Ajoute des étiquettes à l'inventaire, en poursuivant la numérotation.
// Un numéro de suivi déjà présent est ignoré : deux PDF du même envoi ne
// créeront pas deux entrées.
export async function addLabels(nouvelles, attempt = 0) {
  const { rows, rev } = await readInventory();

  const suivisConnus = new Set(
    rows.map((r) => String(r.tracking_number || '').replace(/\s/g, '').toUpperCase())
  );
  const fichiersConnus = new Set(rows.map((r) => r.pdf_file).filter(Boolean));

  let prochain = rows.reduce((max, r) => Math.max(max, Number(r.label_number) || 0), 0);
  const ajoutees = [];

  for (const item of nouvelles) {
    const suivi = String(item.tracking || '').replace(/\s/g, '').toUpperCase();
    if (!suivi || suivisConnus.has(suivi) || fichiersConnus.has(item.pdfFile)) continue;

    prochain += 1;
    const ligne = {
      label_number: String(prochain),
    ...
    rows.push(ligne);
    suivisConnus.add(suivi);
    fichiersConnus.add(item.pdfFile);
    ajoutees.push({ labelNumber: ligne.label_number, tracking: item.tracking, pdfFile: item.pdfFile });
  }

  if (!ajoutees.length) return [];

  try {
    await uploadCSV(INVENTORY_PATH, toCSV(INVENTORY_COLUMNS, rows), rev);
  } catch (err) {
    if (err.isConflict && attempt < 2) return addLabels(nouvelles, attempt + 1);
    throw err;
  }
...
async function fileLabel(labelNumber, orderName, returnId, destination, pdfFile) {
  const original = pdfFile || `${LABEL_PREFIX}${labelNumber}.pdf`;
  const cleanOrder = String(orderName).replace(/^#/, '');
  const source = `${FOLDER_AVAILABLE}/${original}`;
  const target = `${destination}/${cleanOrder}_${returnId}_${original}`;
...
export async function updateReturn(returnId, items, draftOrderId = null, attempt = 0) {
  ...
  // Second verrou, indépendant de l'inventaire : si un article de ce retour
  // porte déjà un statut de réception, le colis est arrivé et la demande ne
  // se modifie plus. Sans ce contrôle, une remise à zéro de l'inventaire
  // rouvrirait des retours déjà traités et dupliquerait leurs articles.
  const dejaRecu = existingItems.some(
    (i) => i.return_id === returnId && i.item_status && i.item_status !== ITEM_REQUESTED
  );
  if (dejaRecu) {
    return { locked: true, status: RETURN_RECEIVED };
  }
...
export async function receiveReturn(returnId, scannedBarcodes = [], attempt = 0) {
  ...
  await fileLabel(label.label_number, label.order_name, returnId, FOLDER_RECEIVED, label.pdf_file);

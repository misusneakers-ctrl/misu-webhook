...
export async function createExchangeDraft({
  ...
  const input = {
    ...
    tags: ['echange', returnId],
    note: `Échange sans frais — retour ${returnId} de la commande ${originOrderName}.`,
    appliedDiscount: {
      valueType: 'PERCENTAGE',
      value: 100,
      title: 'Échange',
      description: `Échange sans frais du retour ${returnId}`
    }
  };
...
// La note est un champ unique : on la relit et on complète, sinon on efface
// ce que Shopify ou Luc y avaient écrit.
export async function appendOrderNote(orderId, addition) {
  const data = await shopifyGraphQL(

      ...
      orders(first: 10, query: $q) {
        edges {
          node {
            ...
            metafields(namespace: "misu", first: 20) {
              nodes { key value }
            }
          }
        }
...
// ou quand la commande vient d'un webhook transporteur authentifié.
...
// Recherche une commande par numéro de suivi transporteur, seule information
// présente dans le payload du webhook Smarty365 (pas de référence de
// commande). Shopify ne propose pas de filtre de recherche sur le numéro de
// suivi : on parcourt donc les commandes expédiées les plus récentes par
// ordre décroissant de traitement, et on compare le numéro de suivi de
// chaque fulfillment. Le webhook arrivant juste après l'expédition, la
// commande correspondante se trouve normalement dans les tout premiers
// résultats.
export async function findOrderByTrackingNumber(trackingNumber, { maxOrders = 250, pageSize = 50 } = {}) {
  const target = String(trackingNumber || '').trim();
  ...
  while (scanned < maxOrders) {
    const data = await shopifyGraphQL(
      `query FindByTracking($first: Int!, $after: String) {
        orders(
          first: $first
          after: $after
    ...
    const match = edges.find(({ node }) =>
      node.fulfillments?.some((f) => f.trackingInfo?.some((t) => t.number === target))
    );
...
// Métachamps de suivi (retour ou transporteur), dans l'espace « misu ».
//
// Sert à la fois aux champs de retour (return_status, return_label,
// exchange_order, origin_order, settlement) et aux champs de suivi
// transporteur posés par le webhook Smarty365 (carrier_status,
// carrier_status_label, carrier_updated_at, carrier_tracking_number,
// carrier_events). Les objets/tableaux sont stockés en JSON pour rester
// exploitables.
